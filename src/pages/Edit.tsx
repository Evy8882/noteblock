...
export default function EditFile() {
    ...
    useEffect(() => {
        if (fields.length === 0) {
            setFields([{ id: crypto.randomUUID(), style: "text", content: "" }]);
        }
    }, [fields]);
    ...
    const handleEnterKey = useCallback(
        (e: React.KeyboardEvent<HTMLInputElement>, field: Field) => {
            ...
            setTimeout(() => {
                const inputElements = document.querySelectorAll<HTMLInputElement>('.field-input');
    ...
    const handleFieldKeyDown = useCallback(
        (e: React.KeyboardEvent<HTMLInputElement>, field: Field) => {
            if (e.key === "Enter" && !e.shiftKey) {
                ...
                e.key === "Delete" &&
                (e.target as HTMLInputElement).selectionStart === field.content.length &&
